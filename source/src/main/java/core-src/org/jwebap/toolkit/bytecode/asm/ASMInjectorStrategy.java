...
public class ASMInjectorStrategy implements InjectorStrategy {
	...
	private static void ini() {
		...
		try {
			findResource = ll.getDeclaredMethod("findResource", new Class[] { String.class });
		...
		try {
			defineClass = ll.getDeclaredMethod("defineClass", new Class[] { String.class, byte[].class, Integer.TYPE, Integer.TYPE });
		} catch (SecurityException e1) {
			e1.printStackTrace();
		} catch (NoSuchMethodException e1) {
			e1.printStackTrace();
	...
	private Class injectInternal(String className, Object hanldeObject) throws InjectException {
		...
		try {
			...
			clazz = defineClass(className, this.resourceLoader, this.definedLoader, hanldeObject);
		} catch (ClassNotFoundException e) {
	...
	public Type genByteCode(byte[] rawClassData) throws Exception {
		...
		InjectClassVisitor injectClassVisitor = new InjectClassVisitor(classInitClassVisitor);
		ASMClassVisitor visitor = new ASMClassVisitor(target, injectClassVisitor);
	 ...
	 * 并且这样对中间件的支持最好，适应复杂的部署模型，比如同一个EAR中的EJB,
	 * 但是有些不采用这种策略的中间件可能遇到问题。
	 * 
	 * @throws ClassNotFoundException
	 * @throws DefineBytecodeException
	 * @throws GenBytecodeException
	 */
	private Class defineClass(String clazz, ClassLoader resourceLoader, ClassLoader definedLoader, Object hanldeObject) throws ClassNotFoundException,
		...
		try {
			...
			URL url = null;
			try {
				url = (URL) findResource.invoke(resourceLoader, new Object[] { clazz.replace('.', '/') + ".class" });
			...
			if (url == null) {
				...
				return defineClass(clazz, url, definedLoader == null ? resourceLoader : definedLoader, hanldeObject);
			}
		}
	...
	private Class defineClass(String className, URL url, ClassLoader definedLoader, Object hanldeObject) throws ClassNotFoundException, GenBytecodeException,
			DefineBytecodeException {

		Assert.assertNotNull(definedLoader, "definedLoader为空.");
		...
		try {
			clazz = defineClass.invoke(definedLoader, new Object[] { className, type.getByteCode(), new Integer(0), new Integer(type.getByteCode().length) });
		} catch (Throwable e) {
			...
			while (urls.hasMoreElements()) {
				...
				if (path.indexOf(".jar") > 0) {
					...
					for (int i = 0; i < files.length; i++) {
						...
						try {
							String className = pckName.substring(0, pckName.length() - 1) + files[i].substring(0, files[i].lastIndexOf('.'));
