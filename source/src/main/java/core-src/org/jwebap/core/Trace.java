 ...
 * <p>
 * 轨迹的定义 轨迹用于封装对性能分析有用的信息（比如上下文的信息，线程堆栈的信息，执 行时间的信息等等）。
 * 在系统中，任何程序的执行都有可能留下轨迹（数据库的调用会留下SQL轨迹，
 * 事务的轨迹，http请求会留下访问的轨迹，程序的方法执行也可以留下轨迹）。
 * 轨迹之间存在关系，比如数据库事务的轨迹和SQL的轨迹，http请求和类方法
 * 执行的轨迹之间都有一定的关系。Jwebap中通过字节码注入，收集运行时的信
...
public class Trace {
	...
	public long getActiveTime() {
		return getInactiveTime() <= 0L ? System.currentTimeMillis() - getCreatedTime() : getInactiveTime() - getCreatedTime();
	}
